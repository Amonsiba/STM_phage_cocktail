...
# Project Summary 
B28B was grown to log phage OD600 of 0.1 and subjected to phage infection of ANB28, SBP2@2, KB824, two phage and three phage cocktails and the following MOIs 0.001, 1, 10. Bacteria and phage lysates were aliqoute in a 96 well plate and read in the Aligent LogPhase 600. Plates used? 
...
```{r loading in data}
#Set Wd
setwd("~/Desktop/Chapter 1_ Kinetics and Characterization/Raw Data for Figures_231208/Figure 5. MOI Growth Curves/Raw_data/")
...
#Combining MTC data
Data_all <- rbind(Ca2, Ca3, Ca4) #need to find the correct way of doing this
#only incorporated 3 plates 
...
KB <- ggplot(sKB_subsetted, aes(x=Time, y=Average, color = MOI))+
  ...
  geom_line(size = 1.5) +
  scale_color_manual(values = c("mediumpurple1", "purple1", "purple4", "black", "gray30")) +
  labs(x="Time(Hours)\n", y ="OD600nm") +
  ggtitle("KB824")+
  theme_classic()+
  theme(
...
#Groups data to include an avereage "blank" column
C1_48 <- Curve1 %>% 
  mutate(Time = round(Time, 1)) %>%
  filter(between(Time,0.0,72.0))
...
C1_sum <- C1_GC %>%
  ...
  dplyr::select(Well, Condition, MOI, auc_l, auc_e, sigma) %>%
  mutate(rep = 1)
...
#Outlier check - no outliers noted (1 out line from a control - will remove repeat for publication analysis)
```
...
#Homogeneity is met - data is evenly distributed around the mean 
```
...
anb.av <- aov(anb.lm)
anova(anb.av) %>% knitr::kable()

kb.lm <- lm(auc_l ~ MOI, data = KBonly)
kb.av <- aov(kb.lm)
anova(kb.av) %>% knitr::kable()
library("report") 
report(kb.av)

sbp.lm <- lm(auc_l ~ MOI, data = SBPonly)
sbp.av <- aov(sbp.lm)
anova(sbp.av) %>% knitr::kable()
report(sbp.av)

c3.lm <- lm(auc_l ~ MOI, data = C3only)
c3.av <- aov(c3.lm)
anova(c3.av) %>% knitr::kable()
...
```{r}
png("../Figures/MOIs_2.png", units="in", width=10, height=6, res=600)
s2 <- ggarrange(ANB,KB, SBP, anb_anova, kb_anova, sbp_anova,
...
moi1_gc <- ggplot(sMOI_1, aes(x=Time, y=Average, color = Input))+
  geom_ribbon(aes(ymin=Average-StDev, ymax = Average+StDev), fill = "#e3e3e3", linetype=0)+
  geom_line(size = 1.5) +
  scale_color_manual(values = c("#FEC000", "black","#EF2402" , "#0420BC", "#B7167E")) +
  labs(x="Time(Hours)", y ="OD600nm") +
  theme_classic()+
  theme(
    plot.title = element_text(size=20, hjust = 0.5, face="bold"),
...
moi1_anova_graph <- MOI_1_auc%>%
  ggplot(aes(x=factor(Input, level=c("B28B", "ANB28", "KB824", "SBP2Φ2", "Cocktail")), y=auc_l, fill = Input)) + 
  geom_violin() +
  geom_jitter() +
  labs(y = "AUC", x = "MOI") +
  theme_classic()+
  ...
  stat_compare_means(method = "anova", label.y = 0) +
  scale_fill_manual(values = c("#FEC000", "black","#EF2402" , "#0420BC", "#B7167E"))
moi1_anova_graph

```

```{r}
figure2 <- ggarrange(moi1_gc, moi1_anova_graph, 
                     labels = c("A", "B"),
                     common.legend = TRUE, legend = "bottom", 
                     align = "h", 
