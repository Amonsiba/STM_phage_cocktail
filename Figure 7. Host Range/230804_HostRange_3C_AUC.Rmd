...
```{r loading in data}
#Set Wd
Curve1 <- read.csv("230804_HostRange_Set3_MOI1_plate1.csv")
Curve2 <- read.csv("230804_HostRange_Set3_MOI1_plate2.csv")
Curve3 <- read.csv("230804_HostRange_Set3_MOI1_plate3.csv")
Curve4 <- read.csv("230804_HostRange_Set3_MOI1_plate4.csv")
```

```{r loading in maps}
platemap1 <- read.csv("230804_Plate1_map.csv")
platemap2 <- read.csv("230804_Plate2_map.csv")
platemap3 <- read.csv("230804_Plate3_map.csv")
platemap4 <- read.csv("230804_Plate4_map.csv")
...
Set1and2.12 <- read.csv("All_cleanauc.12.csv")
Set1and2.20 <- read.csv('All_cleanauc.20.csv')
Set1and2.40 <- read.csv('All_cleanauc.40.csv')
...
png("Figures/Combined_ordered.png", units="in", width=10, height=6, res=1200)
Combined_all %>%
  mutate(name = fct_relevel(Phage, "Cocktail", "SBP2Φ2", "KB824", "ANB28", "Bacteria")) %>%
  ggplot(aes(reorder(Strain, growthpercent), name, fill= growthpercent)) + 
  theme_blank() + geom_tile(color="black", size = 0.25) +
  ggtitle("Area Under the Curve") + labs(fill = 'Growth %')+ ylab("Phage Input")+ xlab(NULL)+
  ...
  annotate("text", x=48, y=5, label= Hour12_df[[2]][[2]], fontface = "bold") +
  annotate("text", x=48, y=4, label= Hour12_df[[2]][[1]], fontface = "bold") +
  annotate("text", x=48, y=3, label= Hour12_df[[2]][[4]], fontface = "bold") +
  annotate("text", x=48, y=2, label= Hour12_df[[2]][[5]], fontface = "bold") +
  annotate("text", x=48, y=1, label= Hour12_df[[2]][[3]], fontface = "bold") +
